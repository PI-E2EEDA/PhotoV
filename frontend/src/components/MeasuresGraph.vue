...
const data = computed(() => {
  ...
  const unit = reverse[0]?.type == MeasureType.Energy ? 'Wh' : 'W'
  return {
    ...
    datasets: [
      { label: 'Solar production - ' + unit, data: reverse.map((m) => m.solar_production), borderColor: solar_production_color },
      { label: 'Solar consumption - ' + unit, data: reverse.map((m) => m.solar_consumption), borderColor: solar_consumption_color },
      { label: 'Grid consumption - ' + unit, data: reverse.map((m) => m.grid_consumption), borderColor: grid_consumption_color }
    ]
  }
})
